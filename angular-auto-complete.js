(function (global, factory) {
    ...
    autoCompleteRenderItemDirective.$inject = ['$compile', '$rootScope', '$sce', '$controller'];
    function autoCompleteRenderItemDirective($compile, $rootScope, $sce, $controller) {
        return {
            restrict: 'E',
            transclude: 'element',
            scope: {},
            ...
            link: function (scope, element) {
                ...
                linkFn(createItemScope(scope), function (clonedElement) {
        ...
        function createItemScope(directiveScope) {
            var itemScope = $rootScope.$new(true);
            // additional logic can be added to this controller if needed
            var itemCtrl = itemScope.itemCtrl = $controller(angular.noop);
            ...
            var deregisterWatchesFn = _.map(['index', 'renderItem', 'searchText'], function (key) {
                return directiveScope.$watch(('ctrl.' + key), function (newVal) {
                    switch (key) {
                        case 'renderItem':
                            // add 'item' property on itemScope for backward compatibility
                            itemCtrl.item = itemScope.item = newVal.data;
                            break;
                        default:
                            itemCtrl[key] = newVal;
                            break;
